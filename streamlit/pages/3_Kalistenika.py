...
if "month_index_cal" in st.session_state:
    st.session_state["month_index_cal"] = 1 
else:
    st.session_state["month_index_cal"] = datetime.datetime.today().month
...
with st.sidebar:   
    ...
    with col_month_start:
        ...
        selected_month_start = st.selectbox('Miesiąc początkowy', month_list_start['month_name_pl'].tolist(), index = st.session_state["month_index_cal"]-1)
